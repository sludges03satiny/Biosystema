// ─────────────────────────────────────────────────────────────
// BIOSYSTEMA — app.js
// Rendering, state management, interactions.
// ─────────────────────────────────────────────────────────────

// ── STATE ─────────────────────────────────────────────────────
let S = {
  view:           "home",   // home | day | system | focus
  dayKey:         null,     // wfh | office | bike
  expanded:       {},       // blockId -> bool
  workOpen:       false,
  principleOpen:  null,
  focusOpen:      null,
};
...
function miniBar() {
  ...
  return `<div class="mini-bar">${WORK_BLOCKS.map(b => {
    ...
    return `<div class="mini-bar-seg" style="left:${left}%;width:${Math.max(width,0.8)}%;background:${CAT[b.cat].color}"></div>`;
  }).join("")}</div>`;
}
...
function blockHTML(block, id) {
  ...
  const data  = sciData(block);
  const open  = !!S.expanded[id];
  const tap   = !!data;
  return `<div class="block${tap?" tappable":""}${open?" open":""}"
    style="border-color:${open ? cat.color+"70" : "var(--border)"};box-shadow:${open ? `0 0 20px ${cat.color}12` : "none"}"
    ${tap ? `onclick="toggleBlock('${id}')"` : ""}>
    <div class="block-row">
      <span class="block-icon">${block.icon}</span>
      <div class="block-meta">
        <div class="block-time">
          <span class="block-time-val">${block.time}</span>
          <span class="block-dur">${block.dur}m</span>
        </div>
        <div class="block-label">${block.label}</div>
        ${block.note ? `<div class="block-note" style="color:${cat.color}">${block.note}</div>` : ""}
      </div>
      <div class="block-right">
        <span class="block-pill" style="background:${cat.dim};border:1px solid ${cat.color}35;color:${cat.color}">${cat.label.toUpperCase()}</span>
        ${tap ? `<span class="block-chevron${open?" open":""}">▾</span>` : ""}
      </div>
    </div>
    ${open && data ? `<div class="block-detail">
...
function innerBlockHTML(block, idx) {
  ...
  const id   = `inner-${idx}`;
  const open = !!S.expanded[id];
  return `<div class="inner-block${open?" open":""}"
    style="border-color:${open ? cat.color+"60" : "var(--border2)"}"
    ${data ? `onclick="toggleBlock('${id}')"` : ""}>
    <div class="inner-block-row">
      <span class="inner-icon">${block.icon}</span>
      <div class="inner-meta">
        <div class="inner-time">
          <span class="inner-time-val">${block.time}</span>
          <span class="inner-dur">${block.dur}m</span>
        </div>
        <div class="inner-label">${block.label}</div>
        ${block.note ? `<div class="inner-note" style="color:${cat.color}">${block.note}</div>` : ""}
      </div>
      <div class="inner-right">
        <span class="inner-pill" style="background:${cat.dim};border:1px solid ${cat.color}30;color:${cat.color}">${cat.label.toUpperCase()}</span>
        ${data ? `<span class="inner-chevron${open?" open":""}">▾</span>` : ""}
      </div>
    </div>
    ${open && data ? `<div class="inner-detail">
...
function workCollapsible() {
  ...
  const accent = "#6366f1";
  return `<div class="work-block${open?" open":""}" style="border-color:${open ? accent+"90" : "var(--border)"};box-shadow:${open ? "0 0 24px #6366f112" : "none"}">
    <div class="work-header" onclick="toggleWork()">
      <div class="block-row">
        <span class="block-icon">💼</span>
        <div class="block-meta">
          <div class="block-time">
            <span class="block-time-val">8:30</span>
            <span class="block-dur">9h 30m</span>
          </div>
          <div class="block-label">Work Day</div>
          ${miniBar()}
          <div class="work-pills">
            ${[["deepwork","5× Deep Work"],["break","3× Break"],["recovery","NSDR"],["nutrition","Lunch"],["anchor","Startup + Shutdown"]]
              .map(([cat,lbl]) => `<span class="work-pill" style="color:${CAT[cat].color}">${lbl}</span>`).join("")}
          </div>
        </div>
        <div class="block-right">
          <span class="block-pill" style="background:${CAT.deepwork.dim};border:1px solid ${CAT.deepwork.color}35;color:${CAT.deepwork.color}">WORK</span>
          <span class="block-chevron${open?" open":""}">▾</span>
        </div>
      </div>
    </div>
    ${open ? `<div class="work-inner">
...
function viewHome() {
  ...
  return `<div class="home">
    <div class="home-greeting">${greeting()} What kind of day is today?</div>
    <div class="home-question">SELECT YOUR DAY</div>
    <div class="day-cards">
...
function viewSystem() {
  ...
  return `<div class="system-wrap">
    <div class="section-label" style="margin-top:20px">REFERENCE</div>
    <p class="system-intro">The science behind every decision. Return here when motivation dips — understanding <em>why</em> is the most durable form of commitment.</p>
    ${PRINCIPLES.map((p,i) => {
      const open = S.principleOpen === i;
      return `<div class="principle${open?" open":""}"
        style="border-color:${open ? p.color+"70" : "var(--border)"};box-shadow:${open ? `0 0 20px ${p.color}10` : "none"}"
...
function viewFocus() {
  ...
  return `<div class="focus-wrap">
    <div class="section-label" style="margin-top:20px">FOCUS MODES</div>
    <p class="focus-intro">Four iPhone Focus Modes that make the right behaviour the environmental default. Science-backed context design — your phone reinforces the system instead of fighting it.</p>
    ${FOCUS_MODES.map((m,i) => {
      const open = S.focusOpen === i;
      return `<div class="focus-mode${open?" open":""}" style="border-color:${open ? m.color+"70" : "var(--border)"}">
        ...
        ${open ? `<div class="focus-body">
          <div class="focus-science" style="background:${m.dim};border:1px solid ${m.color}25">
            <div class="focus-science-label" style="color:${m.color}">THE SCIENCE</div>
            <div class="focus-science-text">${m.science}</div>
          </div>
          <div class="focus-section-label">NOTIFICATIONS</div>
          <div class="focus-detail-text">${m.notifications}</div>
          <div class="focus-section-label">HOME SCREEN</div>
          <div class="focus-detail-text">${m.homeScreen}</div>
          <div class="focus-section-label">SETUP STEPS</div>
          <div class="focus-steps">
            ${m.setup.map((step,j) => `<div class="focus-step">
              <span class="focus-step-num">${j+1}.</span>
              <span class="focus-step-text">${step}</span>
            </div>`).join("")}
          </div>
          ${i === 3 ? `<div class="focus-shortcut">
...
function renderNav() {
  const tabs = [
    { key: "home",   icon: "🏠", label: "HOME",   view: "home" },
    { key: "day",    icon: S.dayKey ? DAYS[S.dayKey].emoji : "📅", label: "TODAY", view: "day" },
    { key: "system", icon: "📖", label: "SYSTEM", view: "system" },
    { key: "focus",  icon: "📵", label: "FOCUS",  view: "focus" },
  ];
  return `<nav class="bottom-nav">
    ${tabs.map(t => `<button class="nav-btn${S.view===t.view?" active":""}" onclick="navTo('${t.view}')">
...
// ── ACTIONS ───────────────────────────────────────────────────
function selectDay(key)     { set({ dayKey: key, view: "day", expanded: {}, workOpen: false }); }
function goHome()           { set({ view: "home" }); }
function toggleWork()       { set({ workOpen: !S.workOpen }); }
function toggleBlock(id)    { set({ expanded: { ...S.expanded, [id]: !S.expanded[id] } }); }
function togglePrinciple(i) { set({ principleOpen: S.principleOpen === i ? null : i }); }
function toggleFocus(i)     { set({ focusOpen: S.focusOpen === i ? null : i }); }

function navTo(view) {
  if (view === "day" && !S.dayKey) { set({ view: "home" }); return; }
  set({ view, expanded: {}, workOpen: false, principleOpen: null, focusOpen: null });
}
